    ...
    <div class="bg-red-50 border-2 border-red-200 rounded-lg shadow-md p-6">
      ...
      <div class="max-h-96 overflow-y-auto">
        ...
        <div
          ...
          :key="item._id"
          @click="navigateToSite(item.siteName)"
          class="bg-white rounded-lg p-3 mb-2 hover:shadow-md transition-shadow border border-red-200 cursor-pointer"
        >
          <div class="flex justify-between items-start">
            <div class="flex-1">
              <p class="font-semibold text-gray-900">Site Name: {{ item.siteName }}</p>
              <p class="text-sm text-gray-700 font-medium">Line Item No: {{ item.sno }}</p>
...
export default {
  ...
  methods: {
    navigateToSite(siteName) {
      this.$router.push(`/site/${siteName}`)
    },
    formatNumber(num) {
